...
- Test: `/Users/daodilyas/dev/iqra-portal/supabase/tests/00_grant_firewall.sql` (Step 7, added by the 2026-07-16 security review)
...
-- Column-grant scope: even on the OWN row, only full_name/phone/locale are
-- updatable — id/created_at forgery dies at the column grant (42501).
select throws_ok(
  $$ update public.profiles set id = 'aaaaaaaa-0000-0000-0000-00000000000e'
     where id = 'aaaaaaaa-0000-0000-0000-000000000002' $$,
  '42501',
  null,
  'user A cannot rewrite id on their own row (column grant)'
);
select throws_ok(
  ...
  '42501',
  null,
  'user A cannot forge created_at on their own row (column grant)'
);
reset role;
...
-- The write asymmetry IS the quarantine model: admin reads all, but admin
-- writes to OTHER users' rows go only through the audited service-role
-- admin module — the RLS update policy stays own-only for everyone.
select lives_ok(
  $$ update public.profiles set full_name = 'Kapret av admin'
     where id = 'aaaaaaaa-0000-0000-0000-000000000003' $$,
  'admin update against another profile runs without matching any row'
);
reset role;
...
select is(
  (select full_name from public.profiles
   where id = 'aaaaaaaa-0000-0000-0000-000000000003'),
  'Bruker B',
  'foreign profile unchanged after admin RLS-path write attempt'
);
...
select throws_ok(
  ...
  '42501',
  null,
  'authenticated user cannot update their own user_roles rows'
);
select throws_ok(
  ...
  '42501',
  null,
  'authenticated user cannot delete their own user_roles rows'
);
reset role;

-- ── Hostile claims: authenticated role with NO sub — fail closed ────
select set_config('request.jwt.claims', '{"role":"authenticated"}', true);
set local role authenticated;
select is((select count(*) from public.profiles), 0::bigint,
  'authenticated with missing sub sees zero profiles');
select is((select count(*) from public.user_roles), 0::bigint,
  'authenticated with missing sub sees zero user_roles');
reset role;
...
- [ ] **Step 7 (added by the 2026-07-16 security review): schema-wide grant firewall**

The review's highest-leverage recommendation: the platform's standing default ACL hands every FUTURE `public` table `TRUNCATE/REFERENCES/TRIGGER/MAINTAIN` and every future sequence `setval` to the api roles. Tasks 4-5 strip these by hand; this test makes the discipline a CI gate — a migration in any later phase that forgets its revoke/grant layer goes red here with the offending object named, without a per-table test existing.

Create `/Users/daodilyas/dev/iqra-portal/supabase/tests/00_grant_firewall.sql`:
...
-- Schema-wide privilege firewall (header gotcha 8, enforced).
-- Sweeps EVERY current and future object in public: a migration that
-- forgets its revoke-all/grant-narrow layer fails here by name.

-- 1. anon holds nothing on any table/view in public.
select is_empty(
...
-- 2. Nothing in public is granted to PUBLIC (grantee oid 0).
select is_empty(
  $$ select c.relname::text, a.privilege_type
     from pg_class c
     cross join lateral aclexplode(coalesce(c.relacl, acldefault('r', c.relowner))) a
     where c.relnamespace = 'public'::regnamespace
       ...
       and a.grantee = 0 $$,
  'no PUBLIC grants on any table or sequence in public');

-- 3. The api roles never hold DDL-adjacent verbs on tables.
select is_empty(
  $$ select c.relname::text, r.rolname, a.privilege_type
     from pg_class c
     cross join lateral aclexplode(coalesce(c.relacl, acldefault('r', c.relowner))) a
     join pg_roles r on r.oid = a.grantee
     where c.relnamespace = 'public'::regnamespace
  ...
  'api roles hold no TRUNCATE/REFERENCES/TRIGGER/MAINTAIN on public tables');

-- 4. anon and authenticated hold nothing on any sequence.
select is_empty(
  $$ select c.relname::text, r.rolname, a.privilege_type
     from pg_class c
  ...
  'anon/authenticated hold zero sequence privileges in public');

-- 5. service_role holds at most USAGE on sequences (no setval abuse).
select is_empty(
  $$ select c.relname::text, a.privilege_type
     from pg_class c
     cross join lateral aclexplode(coalesce(c.relacl, acldefault('S', c.relowner))) a
  ...
  'service_role holds at most USAGE on public sequences');

-- 6. The private schema stays closed to anon.
...
Run `supabase test db` — Expected: `00_grant_firewall.sql .. ok` (6/6) alongside the other files. (In the executed history this step landed together with the review-driven `fix:` commit rather than the Step 6 `feat:` commit.)
...
Expected: `00_...`, `01_...` and `02_...` still pass; `03_audit_log_rls.sql` and `04_settings.sql` FAIL (`relation "public.audit_log" does not exist` / `relation "public.settings" does not exist`).
...
Expected: `Applying migration <timestamp>_audit_and_settings.sql` during reset, then all five test files `ok` (00 firewall, 01-02 identity, 03-04 new), `All tests successful.`
