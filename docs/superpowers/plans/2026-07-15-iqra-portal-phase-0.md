...
This is wall 2 of the two-wall model (spec §6): RLS default-deny. **Default-deny means: `enable row level security` + no permissive policy = nobody reads or writes.** We only add the explicit policies listed below. pgTAP tests are written FIRST and must fail.
...
select col_default_is('public', 'profiles', 'locale', quote_literal('nb') || '::text', 'locale defaults to nb');
...
select is((select count(*) from public.user_roles), 0::bigint, 'anon reads zero user_roles');
reset role;
...
select throws_ok(
  ...
  '42501',
  null,
  'authenticated user cannot insert into user_roles (no policy = deny)'
);
reset role;
...
-- Policies run helper functions as the querying role, so those roles need
-- USAGE on the schema (functions themselves are SECURITY DEFINER).
...
-- Belt and braces on top of RLS: strip write grants at the SQL-grant layer.
-- Supabase default privileges GRANT ALL to anon/authenticated/service_role;
-- append-only demands revoking mutation grants explicitly.
...
-- (service_role keeps INSERT + SELECT: the quarantined admin module writes
-- audit entries directly; authenticated keeps SELECT, gated by the policy below.)
...
-- Writes only via the service-role admin module (Phase 7 UI). Revoking the
-- grants (not just omitting policies) makes write attempts RAISE 42501
-- instead of silently matching zero rows — explicit denial, easier to test.
revoke all on table public.settings from anon;
revoke insert, update, delete, truncate, references, trigger
  on table public.settings from authenticated;
