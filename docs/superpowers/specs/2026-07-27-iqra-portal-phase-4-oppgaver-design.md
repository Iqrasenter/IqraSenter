...
| **D5** | ⛔ **REVERSED — see the rationale cell.** As decided 2026-07-27 this read: «Master-spec non-goal retired. The "no message attachments in v1" line no longer holds. Attachments on **announcements/posts** are approved but land in **Phase 5** with the `announcements` table.» | ⛔ **RETIRED 2026-08-04 by Phase 5 §12 Q14 = (a).** The master spec's non-goal was reinstated: `PRODUCT.md:35` holds, announcements carry no attachments, and **Phase 5 creates no third bucket**. Phase 4's own attachment scope (D4 — assignments and submissions) is unaffected and shipped. Revisit after the pilot. |
| **D6** | **Two private buckets by audience** — `assignments` and `submissions`, both created in Phase 4. ⛔ **Corrected 2026-08-04:** this read «Three private buckets … Phase 5 adds `announcements`»; §12 Q14 = (a) ruled that bucket out and Phase 5 shipped without it. | Each bucket's policy matches exactly one audience, so a policy bug cannot leak across audiences and the tightest wall (children's own work) stays isolated. Defensible in the DPIA. |
...
4. **`npm audit` CI gate — resolved 2026-07-27**, same day as this spec (PR #12, `real` @ `b32f4da`). Now production-scoped with a dated per-advisory allowlist (`scripts/audit-gate.mjs` + `.audit-allowlist.json`); a new advisory or an expired acceptance fails the build. CI on `real` is green again. **⚠ This phase inherits one live obligation from it:** the accepted `sharp`/libvips advisory is justified *only* because the portal serves no user-supplied images today. D4 opens image uploads — if attachments are served through `next/image` they reach libvips with user-controlled files and that justification collapses. §5's signed-URL-straight-from-Storage rule bypasses the optimiser and is what keeps the acceptance honest; **verify it holds at the exit gate**, and re-review the advisory if it does not.
